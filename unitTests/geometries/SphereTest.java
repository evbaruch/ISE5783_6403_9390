...
class SphereTest {
    ...
    public void testFindIntersections() {
        ...
        result = sphere.findIntsersections(new Ray(new Point(-1, 0, 0), new Vector(3, 1, 0)));
        ...
        result = sphere.findIntsersections(new Ray(new Point(1, 0.5, 0), new Vector(3, 1, 0)));
        assertEquals(1, result.size(), "Wrong number of points");
        assertEquals(p2, result.get(0),"p1 Does not give what is expected");

        // TC04: Ray starts after the sphere (0 points)
        assertNull(sphere.findIntsersections(new Ray(new Point(1, 2, 0), new Vector(3, 1, 0))),"ERROR: Ray starts after the sphere");

        // =============== Boundary Values Tests ==================
        // **** Group: Ray's line crosses the sphere (but not the center)
        // TC11: Ray starts at sphere and goes inside (1 points)
        p2 = new Point(1.333333333333333, 0.6666666666666669, 0.6666666666666669);
        result = sphere.findIntsersections(new Ray(new Point(2, 0, 0), new Vector(-1, 1, 1)));
        assertEquals(1, result.size(), "Wrong number of points");
        assertEquals(p2, result.get(0),"p1 Does not give what is expected");

        // TC12: Ray starts at sphere and goes outside (0 points)
        assertNull(sphere.findIntsersections(new Ray(new Point(2, 0, 0), new Vector(1, 1, 1))),"ERROR: Ray starts at sphere and goes outside");
        ...
        result = sphere.findIntsersections(new Ray(new Point(-1, -1, -1), new Vector(2, 1, 1)));
        assertEquals(2, result.size(), "Wrong number of points");
        assertEquals(p1, result.get(0),"p1 Does not give what is expected");
        assertEquals(p2, result.get(1),"p2 Does not give what is expected");
...
//        assertEquals(p2, result.get(0),"p1 Does not give what is expected");
        ...
        assertNull(sphere.findIntsersections(new Ray(new Point(1, -2, 0), new Vector(0, -1, 0))),"ERROR: Ray starts after sphere");

        // **** Group: Ray's line is tangent to the sphere (all tests 0 points)
        // TC19: Ray starts before the tangent point
        assertNull(sphere.findIntsersections(new Ray(new Point(1, 1, -1), new Vector(0, 0, 1))),"ERROR: Ray starts before the tangent point");

        // TC20: Ray starts at the tangent point
        assertNull(sphere.findIntsersections(new Ray(new Point(1, 1, 0), new Vector(0, 0, 1))),"ERROR: Ray starts at the tangent point");

        // TC21: Ray starts after the tangent point
        assertNull(sphere.findIntsersections(new Ray(new Point(1, 1, 1), new Vector(0, 0, 1))),"ERROR: Ray starts after the tangent point");

        // **** Group: Special cases
        // TC19: Ray's line is outside, ray is orthogonal to ray start to sphere's center line
        assertNull(sphere.findIntsersections(new Ray(new Point(1, 2, 0), new Vector(0, 0, 1))),"ERROR: Ray's line is outside");
    }
}